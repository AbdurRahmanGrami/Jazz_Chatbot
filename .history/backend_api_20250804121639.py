...
async def chat_endpoint(request: ChatRequest):
    ...
    logger.info(f"[Chat] Incoming query: {query}")
    ...
    try:
        ...
        for idx, d in enumerate(docs[:3]):  # print only first 3 docs
            logger.info(f"[Doc {idx}] {d.page_content[:300]}...")
        ...
        logger.exception("[Chat] Exception occurred during processing")
