...
def get_session_history(session_id: str, n=3) -> list[dict]:
    cursor.execute("""
    ...
    return [{"role": role, "content": message} for role, message in last_n]
...
def get_user_profile(email: Optional[str]) -> str:
    ...
    try:
        ...
        if row:
            result = json.dumps(dict(zip([col[0] for col in cursor2.description], row)), indent=2)
        conn2.close()
        return result
...
chain = RunnableMap({
    ...
    "context": lambda x: "\n\n".join([
        doc.page_content for doc in retriever.get_relevant_documents(x["message"])
    ])
}) | prompt_template | llm
